...
bool SciTEBase::Run(std::string cmd)
{
	...
	LPWSTR command = const_cast<wchar_t*>(wcmd.c_str());
	if (CreateProcess(NULL, command, NULL, NULL, 0, 0, NULL, NULL, &s_info, &p_info))
	{
...
void SciTEBase::FormatCode(std::string ext)
{
	//Get document string
	std::string docStr = (const char*)wEditor.CallReturnPointer(SCI_GETCHARACTERPOINTER);

	if (ext == "json" || ext == "jsonm")
	{
		Json::Value json;
		if (Json::Reader().parse(docStr, json))
		{
			std::string styled = (ext == "jsonm") ? Json::FastWriter().write(json) : Json::StyledWriter().write(json);
			wEditor.Call(SCI_SETTEXT, 0, reinterpret_cast<sptr_t>(styled.c_str()));
		}
	}
	else if (ext == "cpp" || ext == "cxx" || ext == "hpp" || ext == "c" || ext == "h" || ext == "inl")
	{
		std::string code = CodeFormatter(docStr).ToString();
		wEditor.Call(SCI_SETTEXT, 0, reinterpret_cast<sptr_t>(code.c_str()));
	}
}
