...
SciTEWin::SciTEWin(Extension *ext) : SciTEBase(ext) {
	...
	//read SciTE.ini config
	FilePath config(GetSciteDefaultHome(), FilePath(L"SciTE.ini"));
	if (config.Exists())
	{
		propConfig.Clear();
		propConfig.Read(config, config.Directory(), filter, &importFiles, 0);
	}
...
FilePath SciTEWin::GetDefaultDirectory() {
	if (propConfig.Exists("SciTE_HOME"))
	{
		return FilePath(GUI::StringFromUTF8(propConfig.GetString("SciTE_HOME")));
	}
	else
	{
...
FilePath SciTEWin::GetSciteUserHome() {
	if (propConfig.Exists("SciTE_USERHOME"))
	{
		return FilePath(GUI::StringFromUTF8(propConfig.GetString("SciTE_USERHOME")));
	}
	else
	{
...
void SciTEWin::AddToPopUp(const char *label, int cmd, bool enabled) {
	...
	////1. Load dll in memory
	//LPCWSTR path = L"D:\\Assembla\\Scite\\SciTE\\Debug\\IconLibrary.dll";
	//HMODULE hModule = LoadLibrary(path);

	////2. Load bitmap named IDB_MYBMP1
	//HBITMAP hBitmap = LoadBitmap(hModule, MAKEINTRESOURCE(101));
	//
	//HBITMAP hBitmap2 = (HBITMAP)LoadImage(hInstance, L"D:\\bmp\\a.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);

	//HBITMAP hBitmap3 = (HBITMAP)LoadImage(hModule, MAKEINTRESOURCE(111), IMAGE_BITMAP, 16, 16, LR_LOADTRANSPARENT | LR_LOADMAP3DCOLORS);
	////HBITMAP hBitmap3 = (HBITMAP)LoadImage(hInstance, MAKEINTRESOURCE(111), IMAGE_BITMAP, 16, 16, LR_MONOCHROME);
	////DWORD lastError = GetLastError();
	//int id = 0;
	//HMENU submenu = CreatePopupMenu();
	//AppendMenu(submenu, MF_STRING | MF_ENABLED, id++, L"XP");
	//AppendMenu(submenu, MF_STRING | MF_ENABLED, id++, L"VISTA");
	//AppendMenu(submenu, MF_STRING | MF_ENABLED, id++, L"Win 7");

	//InsertMenu(menu, 4, MF_BYPOSITION | MF_POPUP, UINT(submenu), L"Windows");
	//SetMenuItemBitmaps(menu, 1, MF_BITMAP, hBitmap2, hBitmap2);
	//SetMenuItemBitmaps(menu, 2, MF_BITMAP, hBitmap3, hBitmap2);
	//SetMenuItemBitmaps(menu, 3, MF_BITMAP, hBitmap2, hBitmap2);
